## Hi there 👋 I'm Adhvika

### 👩‍💻 Who am I?
I’m an Electronics and Communication Engineering (ECE) undergraduate who enjoys building things, experimenting beyond the curriculum, and learning by doing. I’m curious about how hardware and software come together in real-world systems, and I love exploring ideas through side projects.

---

### 🔭 What I’m currently working on
- Learning and applying core ECE subjects
- Sensor-based and IoT projects using Arduino and microcontrollers  
- Exploring embedded systems through hands-on experimentation  
- Academic and mini-projects related to signal processing and electronics
- Focus on system integration, debugging, and embedded IoT workflows  
- Improving my coding skills across different programming languages  

---

### 🌱 What I’m currently learning
- IoT and sensor interfacing  
- PCB design fundamentals
- Bare-Metal programming
- Strengthening problem-solving and system-level thinking  

---

### 🛠️ Tools & Technologies I use
- **Programming:** C, Python (basics)  
- **Embedded & IoT:** Arduino IDE, sensors, microcontrollers  
- **Software Tools:** MATLAB, LTspice, MIT App Inventor  
- **Design & CAD:** Fusion 360  
- **Other:** Flowgorithm, basic data handling and signal processing  

---

### 🤝 I’m open to
- Collaborating on beginner-friendly open-source or hardware projects  
- Learning from others and contributing where I can  
- Exploring interdisciplinary projects that combine electronics and software  

---

### ✍️ Beyond tech
When I’m not working on electronics, I enjoy creative writing and poetry, reading, running, and DIY electronics experiments. I believe creativity and engineering go hand in hand.

---

### 📫 How to reach me
- 📧 Email: adhvika.ece@gmail.com  
- 💼 LinkedIn: https://www.linkedin.com/in/adhvika-gedela-a94631332

---

⭐ Thanks for stopping by! Feel free to explore my repositories or connect with me.
  
P.S. most of the repositories contain files built by integrating documented examples, libraries, and iterative testing.

-->
